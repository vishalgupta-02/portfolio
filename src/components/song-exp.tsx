...
function SongTeaser({ song }: SongTeaserProps) {
  return (
    <motion.button
      ...
      className="group relative flex w-full items-center gap-3 overflow-hidden rounded-full text-left outline-none transition-colors"
    >
      {/* Label */}
...
function MiniEqualizer() {
  return (
    <div aria-hidden="true" className="flex h-3 shrink-0 items-end gap-[2px]">
      {bars.map((height, index) => (
        <motion.span
          ...
          className="block w-[2px] rounded-full bg-green-400"
        />
      ))}
    </div>
  );
